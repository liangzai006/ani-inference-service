...
func ValidateCreate(in CreateRequest) error {
	...
	if mode == "" || mode == "deployment" {
		if in.Replicas < 1 {
			return fmt.Errorf("replicas must be positive for deployment runtime")
		}
		...
		if in.Replicas < 1 || in.WorkerReplicas < 1 {
			return fmt.Errorf("leader_worker_set requires replicas >= 1 and worker_replicas >= 1")
		}
	} else {
